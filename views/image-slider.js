...
html`
    ...
    <div class="mobile-slider-view">
        ${slides.map(x => html`${mobileConponent(x)}`)}
    </div>`;


let mobileConponent = (slide) => 
html`<div class="mob-container">
        <div class="header-container-mob">
            <h3>${slide.title.toUpperCase()}<h3>
        </div>
        <div class="img-mob-container">
            <img class="mob-img" src=${slide.asset}>
        </div>
        <div class="btn-mob-container">
            <a  href=${slide.href}><button class="mob-img-button">${slide.href.slice(1).toUpperCase()}</button></a>
        </div>
</div>`;